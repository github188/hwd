            ...
            if (key == 'country@%city') {
                ...
                if (!isEmptyObject(value)) {
                    $.each(value, function (name, countryObj) {
                        var hid = 'country' + CheckboxId_SEPARATOR + name,
                            countryLi = $('<li class="facet-value"></li>'),
                            cInput = $('<input type="checkbox" class="country-collapse">').attr({
                                /*'id': hid,*/
                                'name': hid
                            }),
                            cDiv = $('<div class="label-container"></div>')
                                .append($('<span class="facet-count"></span>').html(''))
                                .append($('<label class="facet-label"></label>')
                                    .attr({
                                        'for': hid,
                                        'title': hid
                                    })
                        ...
                        cInput.on('click', function () {
                            $('#collapse' + this.id.split(CheckboxId_SEPARATOR)[1]).collapse('toggle');
                        });
                        countryLi.append(cInput).append(cDiv).appendTo($country);

                        var total = countryObj['count'],
                            citiesContainer = $('<div class="collapse" id="collapse' + name + '"></div>').appendTo(countryLi),
                            $cities = $('<ol class="inner-facet-values"></ol>').appendTo(citiesContainer);

                        $cities.append(genSidebarLi('city_'+name, '全选', countryObj['count']));//全部
