...
<div class="main">
    <div class="wraper">
        <p class="tr"><a href="javascript:history.back() " class="blue ml10 fz12">返回注册页&raquo;</a></p>
...
2.网易通行证服务简介
本服务条款所称的“网易通行证”是指用户注册的合法、有效的帐号，包括
...
考虑到网易产品服务的重要性，用户同意：
（1）提供及时、详尽及准确的个人资料。
（2）不断更新注册资料，符合及时、详尽准确的要求。网易通行证注册资料包括：用户的网易通行证帐号、密码及注册通行证或更新通行证注册资料时输入的所有信息。用户注册通行证账号时填写的身份证件信息不能更新。
如果用户提供给网易公司的资料不准确，不真实，不合法有效，网易公司保留终止用户使用网易各项服务的权利。用户同意，其提供的真实、准确的网易通行证注册资料作为认定用户与其网易通行证帐号的关联性以及用户身份的唯一证据。用户在享用网易各项服务的同时，同意接受网易提供的各类信息服务。网易公司提醒用户，用户注册通行证账号或更新注册信息时填写的证件号码，在注册通行证账号成功或补充填写后将无法进行修改，请用户慎重填写各类注册信息。
用户同意与注册、使用网易通行证帐号相关的一切资料、数据和记录，包括但不限于通行证帐号名称、注册信息、所有登录、消费记录和相关的使用统计数字等归广州网易计算机系统有限公司所有。发生争议时，用户同意以广州网易计算机系统有限公司的系统数据为准，广州网易计算机系统有限公司保证该数据的真实性。
尽管有前述约定，对于用户使用通行证帐号享受网易公司旗下单项服务（如游戏产品、网易宝产品等）时产生的一切数据，包括但不限于产品登录记录、消费记录及其他产品日志、产品客户服务记录、用户在产品中创造的社会网络内容等，归具体产品运营主体所有。发生争议时，用户同意以具体产品运营主体的系统数据为准。但是如果单项条款存在与前述不同的约定，则以单项条款约定为准。
3.服务条款的修改
...
（6）用户应遵守网易的所有其他规定和程序。
用户须对自己在使用网易服务过程中的行为承担法律责任。用户承担法律责任的形式包括但不限于：对受到侵害者进行赔偿，以及在网易公司首先承担了因用户行为导致的行政处罚或侵权损害赔偿责任后，用户应给予网易公司等额的赔偿。若用户违反以上规定，网易有权作出独立判断立即暂停或终止对用户提供部分或全部服务，包括冻结、取消用户服务帐号等措施。用户理解，如果网易发现其网站传输的信息明显属于上段第(1)条所列内容之一，依据中国法律，网易有义务立即停止传输，保存有关记录，向国家有关机关报告，并且删除含有该内容的地址、目录或关闭服务器。
...
11.通知
所有发给用户的通知都可通过电子邮件、常规的信件或在网站显著位置公告的方式进行传送。网易公司将通过上述方法之一将消息传递给用户，告知他们服务条款的修改、服务变更、或其它重要事情。同时，网易公司保留对申请了网易邮箱的用户投放商业性广告的权利。
12.内容、商标的所有权
...
如出现纠纷，用户和网易公司一致同意将纠纷交由广州网易计算机系统有限公司所在地法院管辖。
14.信息储存及相关知识产权
网易公司对通行证上所有服务将尽力维护其安全性及方便性，但对服务中出现的信息（包括但不限于用户发布的信息）删除或储存失败不承担任何责任。另外网易公司有权判定用户的行为是否符合本服务条款的要求，如果用户违背了本服务条款的规定，网易公司有权中止或者终止对其网易通行证帐号的服务。
网易公司尊重知识产权并注重保护用户享有的各项权利。在网易通行证所含服务中，用户可能需要通过上传、发布等各种方式向网易公司提供内容。在此情况下，用户仍然享有此等内容的完整知识产权。用户在提供内容时将授予网易公司一项全球性的免费许可，允许网易公司使用、传播、复制、修改、再许可、翻译、创建衍生作品、出版、表演及展示此等内容。
15.青少年用户特别提示
青少年用户必须遵守全国青少年网络文明公约：
要善于网上学习，不浏览不良信息；要诚实友好交流，不侮辱欺诈他人；要增强自护意识，不随意约会网友；要维护网络安全，不破坏网络秩序；要有益身心健康，不沉溺虚拟时空。
16.网易通行证帐号的有效期
用户清楚知悉网易通行证帐号存在有效期，并同意不定时登录使用网易通行证帐号以延续其有效期。
一般情况：
...
特别情况：
（3）如果用户以网易收费邮箱注册的网易通行证帐号不存在网易充值一卡通点数，且该帐号下的邮箱在宽限期满后仍未续费的，网易公司有权立即删除该通行证帐号、终止该帐号下的邮箱服务并将邮箱中的内容删除；
（4）如果用户以网易收费邮箱注册的网易通行证帐号下存在网易充值一卡通点数，且该帐号下的邮箱在宽限期满后仍未续费的，网易公司有权立即终止该帐号下的邮箱服务并将邮箱中的内容删除；如该帐号连续540天没有登录，则该帐号下的网易充值一卡通点数自该帐号最后一次登录之日起的第540天24时到期作废，网易公司有权删除该网易通行证帐号。
当帐号被删除后，该网易通行证帐号的所有资料以及与该网易通行证帐号相关的全部服务资料和数据（包括但不限于邮箱信息、网易充值一卡通点数信息、游戏帐号信息，等）将同时删除，且不可恢复。该帐号名有可能会被新的用户注册。
登录网易通行证是指通过网易通行证帐号密码认证，包括但不限于如下方式：
(1)通过Web方式登录；
(2) 通过邮件，游戏和手机应用等客户端软件登录；
(3)通过网易通行证帐号登录网易POPO。
网易充值一卡通是指：网易公司发行的一种带有点数的一次性预付卡，供用户消费网易公司所有可以用点数支付的收费产品，计算单位为“点”。
网易充值一卡通点数到期作废是指：网易通行证帐号下存在的网易充值一卡通点数将不能被用户使用，该帐号下的网易充值一卡通点数记录被清空，并且不可以恢复。网易公司也不会对此做出任何的补偿或者赔偿。
17.其他
